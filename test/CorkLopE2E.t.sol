...
// ConstraintRateAdapter, WhitelistManager). Only the MarketRegistry's wrapper factory is
// mocked (it returns phoenix's RateOracleMock as the pair's rate-oracle wrapper).
//
...
//    soundness no longer depends on the rate holding still between setUp and the fill.
//
...
contract CorkLopE2ETest is BaseTest {
    ...
    /// @dev The rate the constraint is derived from at signing time, carried in `additionalData` and
    ...
    // Plain taker lifting the ASK — never signs anything.
    ...
    function setUp() public override {
        ...
        // `"USD"` is seeded as a denomination label by the registry constructor and its unit IS the
        // US Dollar sentinel, so each source resolves in ZERO bridge hops and no conversion feed has
        // to be added before these two writes.
        ...
        // The adapter creates pools through the real controller: grant it the creator role.
        ...
        vm.prank(bravo);
        defaultCorkController.grantRole(poolCreatorRole, address(hook));
    ...
    ///      deploys/records. Because nothing here reads the rate, the pool id this ceremony predicts is
    ///      stable however far the rate moves between now and the fill.
    ...
    ///      inside the fill, record what the order/permit signatures must commit to, then roll
    ///      the state back. Sound because SharesFactory deploys the share tokens with CREATE2
    ///      (salt = pool id, constructor args derived from the market params), so the replayed
    ///      creation inside the real fill lands on the same addresses regardless of any nonce
    ///      drift between now and then.
    function _predictJitPool() internal {
        Market memory m = _derivedMarket();
        jitPoolId = corkPoolManager.getId(m);

        uint256 snapshot = vm.snapshotState();
        vm.prank(address(hook));
        defaultCorkController.createNewPool(
            IDefaultCorkController.PoolCreationParams({
                pool: m, unwindSwapFeePercentage: UNWIND_FEE, swapFeePercentage: SWAP_FEE, isWhitelistEnabled: false
            })
        );
        (address cpt, address cst) = corkPoolManager.shares(jitPoolId);
    ...
    {
        return abi.encode(
            CorkLimitOrderAdapter.JITMarketParams({
                collateralAsset: address(usdc),
                referenceAsset: address(wstEth),
                expiryTimestamp: jitExpiry,
